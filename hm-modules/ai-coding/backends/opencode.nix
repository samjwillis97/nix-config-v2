...
let
  ...
  resolveModel =
    ...
    if alias == null then
      null
    else if cfg.modelAliases ? ${alias} then
      cfg.modelAliases.${alias}.opencode
    else
      alias;

  # Map DSL transport type to OpenCode transport type
  mapTransportType = type: if type == "stdio" then "local" else "remote";
  ...
  allMcpServers =
    ...
    in
    filterAttrs (name: _: !(builtins.elem name ocCfg.disabledMcpServers)) merged;
  ...
  opencodeSandboxed = agentSandbox.mkSandbox {
    ...
    rwFiles = cfg.sandbox.extraStateFiles ++ sandboxCfg.extraStateFiles;
    env = cfg.sandbox.extraEnv // sandboxCfg.extraEnv;
